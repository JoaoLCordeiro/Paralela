...
void* PrefixSumPart (void* argumentos){
	long int*	argum = (long int*) argumentos;
...
void* sumMaxPrefix (void* argumentos){
	long int*	argum = (long int*) argumentos;
...
long int* PthPrefixSum (long int* vEntrada, long int nTotalElements, long int nThreads){
	...
	long int* vSaida	= (long int *) malloc (nTotalElements*sizeof(long int));
	long int* vMaximos	= (long int *) malloc (nThreads*sizeof(long int));
	...
	for (int i = 0 ; i < nThreads ; i++){
		long int* argumentos = (long int *) malloc (5 * sizeof(long int));
...
int main (int argc, char *argv[]){
	...
	//correcao de erros
	long int *InputVector = (long int *) malloc (nTotalElements*sizeof(long int));
